...
import { TransformOptions, FullSlug, transformLink } from "../../util/path"
...
function replaceMatches(str: string, regex: RegExp, replacer: (match: RegExpExecArray) => string) {
  ...
  for (const match of matches) accumulator = accumulator.replace(match[0], replacer(match))
  return accumulator
}
...
function wikilinkReplacement(currentSlug: FullSlug, transformOptions: TransformOptions) {
  return (match: RegExpExecArray) => {
    ...
    if (link) {
      let url = transformLink(
        currentSlug,
        encodeURI(decode(link.trim() + (fragment ? "#" + fragment.trim() : ""))),
        transformOptions,
      )
      ...
      if (/\.(png|jpg|jpeg|gif|bmp|svg|webp)$/.test(link)) {
        ...
        const width = imageEmbedMatch?.groups?.width
          ? imageEmbedMatch?.groups?.width + "px"
          : "auto"
        const height = imageEmbedMatch?.groups?.height
          ? imageEmbedMatch?.groups?.height + "px"
          : "auto"
    ...
    markdownPlugins(ctx) {
      ...
      function processMindmapFile(file: any, transformer: Transformer, slug: FullSlug) {
        ...
        recurseChildren((node) => {
          ...
          node.content = replaceMatches(node.content, youtubeImageRegex, ytLinkReplacement())
          node.content = replaceMatches(node.content, calloutRegex, calloutReplacement())
        })(root)
